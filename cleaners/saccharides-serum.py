...
RAW_FILE_NAME = "LC-MS SACCHARIDES SERUM QUANTIATIVE AUTH.xlsx"

df = pd.read_excel(os.path.join(Settings().getdir(SettingsKeys.RAWDIR), RAW_FILE_NAME))
print(df)
print(df.columns)

df[['patient', 'visit']] = df['ID PARTICIPANT / C COMPOUNDS (μΜ)'].str.extract(r'CD_(\d+)_V(\d)')
df['patient'] = 'CD-' + df['patient']
df['visit'] = 'V' + df['visit']

df = df.drop(columns="ID PARTICIPANT / C COMPOUNDS (μΜ)")
...
metadata = DataFrameMetadata(df, "1.3", comment="Mass spectometry, saccharides, serum, pd.NA indicates smaller than measurable values, measured at AUTH",
                             categorical_features=["visit"],
                             column_units={col: "μΜ" for col in float_cols})

save(metadata, "saccharides-serum")
